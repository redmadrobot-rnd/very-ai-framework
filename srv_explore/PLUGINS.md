...
`toggle` нужен, когда спрятать креды недостаточно. Так у `docker`: прокси слушает
loopback, а loopback песочнице разрешён — без остановки контейнера выключенный
...
| тумблер On/Off | зовёт `toggle(ctx, enabled)`, если плагин его объявил, и только потом меняет состояние; исключение оттуда отменяет переключение |

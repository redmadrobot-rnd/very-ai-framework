...
```mermaid
flowchart LR
  ADM["Admin"] -->|"Install · On/Off"| P["плагин<br/>(postgres, docker, …)"]
  P -->|"заводит read-only<br/>доступ, проверяет его"| R[("ресурс:<br/>БД · docker · …")]
  P -.->|"ro-креды<br/>пока On"| A["Agent<br/>в песочнице"]
  A -->|"только чтение"| R
```

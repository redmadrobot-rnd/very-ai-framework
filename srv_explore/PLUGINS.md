...
```mermaid
flowchart LR
  F["Install<br/>форма плагина"] --> I["чеклист установки<br/>…"]
  I --> D["последний шаг:<br/>запись отбита ✓"]
  D --> C[("креды на сервере<br/>ro-DSN · DOCKER_HOST")]
  C -->|"тумблер On"| E["окружение агента<br/>$PG_INSPECTOR_DSN …"]
```

Как этим пользоваться (админка → «Плагины»):

- **Install** — плагин спросит, что ему нужно (например, админский DSN — он
  используется один раз и не сохраняется), прогонит чеклист и покажет, где встало.
- **Тумблер On/Off** — выдавать ли креды агенту. По умолчанию всё выключено.
- **Снять** — убрать установку и забыть креды.
...
**Создание роли — тоже идемпотентное**, с фиксированным именем (`srvx_readonly`):
«есть — сменить пароль, нет — создать». Иначе повторный Install плодит юзеров.

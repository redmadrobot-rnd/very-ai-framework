...
`install.sh` при первой установке печатает **админ-токен** (`adm_…`) — сохрани, второй
раз он не покажется. Штатно то же делает воркфлоу `Deploy srv-explore (host service)`.
...
Поллить раз в 2–5 секунд. Ответ — **факты**, выводы делает вызывающий агент.
Подробности — скилл `.claude/skills/srv-explore/`.
...
| `token_store.py`, `tunnel_keys.py` | токены инженеров, ключи туннеля |
...
| **On / Off** | выдавать эти креды агенту или нет. Установку не трогает |
...
записать и обязан получить отказ. Прошло — установка считается неуспешной.
...
| `rabbitmq` | monitoring-юзер, пустые config/write | `declare` → `access refused` |
...
| задача вернула `[agent прерван]` | упёрлись в `RuntimeMaxSec`; собранные шаги сохранены |

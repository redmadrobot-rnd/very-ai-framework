...
порта нет. Ставится и обновляется деплой-воркфлоу (`install.sh` + systemd-юнит), от
остального проекта не зависит.

> Схема — один файл `src/srv_explore/web/flow.svg`: он же отдаётся на странице агента,
...
**Что доступно:** файлы, логи и journald — всегда; контейнеры, БД, кеши и очереди —
через плагины, которые включает администратор (см. [принципы](#принципы-работы)).
...
Спросить сервер из браузера (например, проверить только что установленный плагин) —
на странице `/` тем же админ-токеном, отдельного инженерного не нужно.
...
| Вкладка | Что там |
|---|---|
| **Что это** | схема, quick start и принципы работы. Открыта без токена |
| **Мой доступ** | по токену: метка и роль, статус ресурсов (`доступен` / `выключен` / `не настроен` — только просмотр, переключает админка), своя история прогонов с командами |
| **Спросить сервер** | тот же агент, что по MCP: вопрос → шаги → факты |

Принимаются оба токена: инженерный `srvx_` и админский `adm_` — роль решает, что видно
(админ видит все прогоны и ссылку в админку, инженер только свои). Токен лежит в
`localStorage` — вход переживает перезапуск браузера, стирает его «Выйти». Не cookie
намеренно: куку браузер прикладывал бы к каждому запросу сам, и появился бы CSRF —
заголовок `Authorization` со стороннего сайта не подделать. Чужой прогон инженеру
отдаётся как несуществующий.
...
Главный принцип: **границы держит ресурс-слой, а не разбор команд.** Агенту не
запрещают писать — ему физически нечем.

```mermaid
flowchart LR
  A["Agent tries<br/>to change something"] --> F{"resource<br/>reachable?"}
  F -->|file| X1["RO-FS<br/>EROFS"]
  F -->|DB| X2["read-only role<br/>permission denied"]
  F -->|docker| X3["socket-proxy POST=0<br/>403"]
  F -->|outbound| X4["egress firewall<br/>+ allowlist"]
  F -->|privileges| X5["unprivileged user<br/>NoNewPrivileges"]
```

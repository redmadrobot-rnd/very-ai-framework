...
Там же **«Настройки»** — куда агенту можно наружу: домены (ходит через прокси, применяется
сразу) и подсети с адресами (напрямую, со следующего прогона). Отдельных ENV-переменных на
это не завёл — env остаётся для деплойных значений, они складываются с заданными в админке.

...
ни от чего в основном проекте не зависит. Концепция — `docs/concept.md`, прайор-арт —
`docs/research.md`. Как подключиться из проекта — скилл `.claude/skills/srv-explore/`.
...
открытый интернет — TLS за reverse-proxy) · `SRV_EXPLORE_GUARD`/`PROMPT`/`AUDIT`/
`TOKENS`/`CWD` · `CLAUDE_CODE_OAUTH_TOKEN` (авторизация модели, секрет, пишет деплой).
...
- **Ещё НЕ прогнано:** деплой-воркфлоу (нужен мерж в main, `workflow_dispatch` виден только
  с дефолтной ветки); TLS/reverse-proxy и readonly-роль БД — под конкретное окружение.

...
Чтобы не держать терминал открытым — постоянный туннель через `~/.ssh/config`:
...
Дальше `ssh -N srvx` (или `autossh -M0 -N srvx` для авто-переподключения) поднимает проброс.
...
- **БД (postgres)** — режим Б: DSN — **обязательное условие включения**. Админ вставляет
  **одноразовый admin-DSN**; сервис создаёт read-only роль `srvx_readonly`
  (`pg_read_all_data`), собирает её ro-DSN, кладёт в `provisioned.json` (StateDir,
  root-only; агент получает его в env только на время прогона), admin-DSN **выкидывает**.
  Повторный заход видит выданные креды и DSN не спрашивает. Выключение — забыть ro-DSN
  (роль в БД остаётся, безвредна).
- **mongo/redis/rabbitmq** — конфиг есть, драйвер провижининга ещё нет → в `/admin`
  тумблер заблокирован («скоро»), включить нельзя (не «включился в тупик»).

Индикатор готовности снимается **живой пробой один раз при включении** (и по кнопке ⟳),
результат кэшируется — показ `/admin` БД не дёргает. Проба строится на `VERIFY` —
**команде-нарушителе**, которая при верной настройке ДОЛЖНА упасть: `ok` — отбита
(read-only держит), `broken` — прошла (запись возможна, тревога), `error` — не подключились
/ иная ошибка, `setup` — включён, но без кредов, `off` — выключен.

...
записать и обязан получить отказ. Прошло — установка считается неуспешной. Исключение
одно: у `rabbitmq` на хосте нет AMQP-клиента, поэтому там вместо пробы записью
вычитываются применённые права.
...
| `rabbitmq` | monitoring-юзер, пустые config/write | права вычитываются с брокера: `config/write = ^$` |

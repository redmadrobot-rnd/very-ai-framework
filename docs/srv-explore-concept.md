...
и проактивно `uniq IN OUT` (второй аргумент = файл записи). Все закрыты и
зафиксированы в `tests/test_srv_explore_guard.py` (110 кейсов). Урок: **разрешить имя
команды недостаточно — нужен per-command контроль опасных флагов + нормализация
`--flag=value` + allowlist для curl/ssh**.

...
DNS-эксфильтрация. Codex-ревью (PR #45) добавило ещё 3: несколько `-c` в psql
(исполняются все, проверялся последний), `ss -K` (убивает сокеты) и `journalctl
--vacuum/--rotate` (чистит журнал) как generic-команды без фильтра флагов, `find
-fprint0` (мимо deny-набора). Все закрыты и зафиксированы в
`tests/test_srv_explore_guard.py` (101 кейс). Урок: **разрешить имя команды
недостаточно — нужен per-command контроль опасных флагов + allowlist для curl/ssh**.

...
#                              ИЛИ d начинается с services/<имя>/);
...
for f in "${changed[@]}"; do
  ...
  case "$f" in
    services/*/*)
      # файл сервиса → тест-каталоги этого сервиса. Матчим по сегменту
      # services/<имя> (тест-каталог = корень сервиса ИЛИ вложенный, файл — любой).
      rest=${f#services/}     # "<имя>/..."
      name=${rest%%/*}        # "<имя>"
